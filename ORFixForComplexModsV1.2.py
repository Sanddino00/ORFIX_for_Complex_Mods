...
def process_block_full(block, section_name, clean_only=False):
    ...
    clean_only=True => remove misplaced runs only, do not rename or insert runs
    clean_only=False => full processing
    """
    if not block:
        return block, []
    ...
    for line in block:
        ...
        # Rename ps-t0 -> ps-t1 if Extra+Diffuse and not clean_only
    ...
    # Insert run line if not clean_only
    ...
    for line in lines:
        ...
        if stripped.startswith("[CommandList") or stripped.startswith("[TextureOverride"):
            if block_lines:
                ...
                clean_only = inside_auto_excluded or inside_manual_excluded
...
def main():
    ...
    # Ask manual exclusions for sections not auto-excluded
